...
- ALWAYS use exact table names and column names from the Lead Prioritizer Dashboard data dictionary.
...
* If user asks questions which include multiple dimensions, add all dimensions in groupby 
eg. What is the premium acquired by state by product -> Group by state, product* 
---
6. ORDER BY RULES
...
- Multiple possible interpretations → Make the best direct match (demo_il_lp_dashboard_data by default), but confirm assumptions.
---
...
- Selected table based on metric (demo_il_lp_dashboard_data)
...
- Average lead score by product
 -> select
    ...
    from lead_prioritizer.demo_il_lp_dashboard_data
    group by product
...
- Lead score quartiles (25% bin) by product
 -> select
    ...
    limit 10;
- Total premium acquired by State by Product?
 -> select
    state_cd,
    sum(premium) as total_premium
    from lead_prioritizer.demo_il_lp_dashboard_data
    group by state_cd, product
    order by total_premium desc
    limit 10;
---
...
- “All products included.”
***Always follow these rules. No deviations. Only use approved tables and columns (demo_il_lp_dashboard_data).***
"""
